...
const CalandarMenu = ({ anchorEl, handleClose }) => {
  const { firstMonth: firstIso, secondMonth: secondIso } = useSelector(state => state.months);
const firstMonth  = new Date(firstIso);
const secondMonth = new Date(secondIso);
  ...
  const isPreviousMonthDisabled =
    new Date(firstMonth).getMonth() === new Date().getFullYear() &&
    new Date(firstMonth).getMonth() < new Date().getMonth();
  const maxForward =
    new Date(secondMonth).getFullYear() === new Date().getFullYear() + 1 &&
    new Date(secondMonth).getFullYear() === new Date().getMonth();
...
>
      <Grid
        ...
        sx={{ p: "24px", gap: { lg: "80px", xs: "60px" }, flexWrap:{ md: "nowrap", xs: "wrap" }  }}
        alignItems='stretch'
        justifyContent='center'
      >
